...
public class ForegroundService extends JobIntentService {
    ...
    private boolean isIden ;//是否已经认证

    static void enqueueWork(Context context) {
        enqueueWork(context, ForegroundService.class, 123, new Intent());
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            thread.join();
            if(isIden){
                Toast.makeText(getApplicationContext(),"当前网络并不需要认证",Toast.LENGTH_SHORT).show();
            }
    ...
    public void  setForegroundService()
    {
        ...
        channel.setDescription("用于后台启动服务");
        //在创建的通知渠道上发送通知
    ...
    synchronized private void getLock(Context context){
        if(mWakeLock==null){
            ...
            mWakeLock=mgr.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, ForegroundService.class.getName());
